...
public class TermuxTerminalSessionActivityClient extends TermuxTerminalSessionClientBase {
    ...
    public void onStart() {
        ...
        final TermuxService service = mActivity.getTermuxService();
        if (service != null) {
    ...
    public void onSessionFinished(@NonNull TerminalSession finishedSession) {
        ...
        final int index = service.getIndexOfSession(finishedSession);
        ...
        final TermuxSession termuxSession = service.getTermuxSession(index);
        ...
        if (mActivity.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LEANBACK)) {
            ...
            final int exitStatus = finishedSession.getExitStatus();
            if (exitStatus == 0 || exitStatus == 130 || isPluginExecutionCommandWithPendingResult) {
                removeFinishedSession(finishedSession);
            }
        }
    }
    ...
    public void onPasteTextFromClipboard(@Nullable TerminalSession session) {
        ...
        final String text = ShareUtils.getTextStringFromClipboardIfSet(mActivity, true);
        if (text != null)
    ...
    public void setTerminalShellPid(@NonNull TerminalSession terminalSession, int pid) {
        ...
        final TermuxSession termuxSession = service.getTermuxSessionForTerminalSession(terminalSession);
        if (termuxSession != null)
    ...
    public void switchToSession(boolean forward) {
        ...
        final TerminalSession currentTerminalSession = mActivity.getCurrentSession();
    ...
    private void renameSession(TerminalSession sessionToRename, String text) {
        ...
        final TermuxService service = mActivity.getTermuxService();
        if (service != null) {
            final TermuxSession termuxSession = service.getTermuxSessionForTerminalSession(sessionToRename);
            if (termuxSession != null)
    ...
    public void addNewSession(boolean isFailSafe, String sessionName) {
        ...
        if (service.getTermuxSessionsSize() >= MAX_SESSIONS) {
            ...
            final TermuxSession newTermuxSession = service.createTermuxSession(null, null, null, workingDirectory, isFailSafe, sessionName);
            if (newTermuxSession == null) return;

            final TerminalSession newTerminalSession = newTermuxSession.getTerminalSession();
    ...
    public TerminalSession getCurrentStoredSessionOrLast() {
        final TerminalSession stored = getCurrentStoredSession();
        ...
        if (stored != null) {
            ...
            final TermuxSession termuxSession = service.getLastTermuxSession();
            if (termuxSession != null)
    ...
    private TerminalSession getCurrentStoredSession() {
        final String sessionHandle = mActivity.getPreferences().getCurrentSession();
    ...
    String toToastTitle(TerminalSession session) {
        ...
        if (!TextUtils.isEmpty(session.mSessionName)) {
            // Optimized: Use char append instead of String for single character
    ...
    public void checkForFontAndColors() {
        try {
            final File colorsFile = TermuxConstants.TERMUX_COLOR_PROPERTIES_FILE;
